  ...
  Memory.run st (fun () ->
    match
      Sensors.attach
        ...
        ~purpose:"Electricity" ~meter_type:Sensor.Counter ~unit:"kWh" ~resample_minutes:15
  ...
  Memory.run st (fun () ->
    match
      Sensors.attach
        ...
        ~purpose:"Electricity" ~meter_type:Sensor.Counter ~resample_minutes:15 ()
    with
...
let list_active_returns_attached () =
  ...
  Memory.run st (fun () ->
    let _ = Sensors.attach ~parent:bldg
              ...
              ~meter_type:Sensor.Counter ~resample_minutes:15 () in
    ...
    let s =
      match
        Sensors.attach ~parent:bldg
          ...
          ~meter_type:Sensor.Counter ~resample_minutes:15 ()
      with
      | Ok x -> x
      | Error e -> Alcotest.failf "attach: %s" (Errors.message e)
    in
      ...
      let s3 =
        match
          Sensors.attach ~parent:bldg
            ...
            ~meter_type:Sensor.Counter ~resample_minutes:15 ~formula:formula_s3 ()
        with
        | Ok x -> x
    ...
    Memory.run st (fun () ->
      match
        Sensors.attach ~parent:bldg
          ...
          ~meter_type:Sensor.Counter ~resample_minutes:15 ~formula:Formula.Zero ()
      with
      | Ok x -> x
      | Error e -> Alcotest.failf "attach: %s" (Errors.message e))
  in
