        ...
        li class="sensor-item sensor-row" {
            ...
            details class="kebab" {
                ...
                div class="action-dropdown kebab__menu" {
                    ...
                    // Soft-navigate (Astro view transition) so the sidebar/tree is
                    // preserved. /measurements re-reads location.search on astro:page-load
                    // (see its <script>), so it works without a full page reload.
