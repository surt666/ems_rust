...
def test_ancestor_keys_full_depth():
    ...
    assert keys == [
        ...
        "HN2#2|HN3#9",
        "HN2#2|HN3#9|HN4#456",
        "HN2#2|HN3#9|HN4#456|L#10009",
    ]


def test_ancestor_keys_meter_directly_under_company():
    keys = m.ancestor_keys([2, None, None, None, None, None, None, None], 10009)
    assert keys == ["HN2#2", "HN2#2|L#10009"]


def test_build_sk_company_and_node_and_leaf():
    assert m.build_sk("HN2#2", "Electricity", "d", "2026-06-07") == "HN2#2#Electricity#d#2026-06-07"
    assert m.build_sk("HN2#2|HN3#9|HN4#456", "Electricity", "d", "2026-06-07") == \
        "HN2#2|HN3#9|HN4#456#Electricity#d#2026-06-07"
    assert m.build_sk("HN2#2|HN3#9|HN4#456|L#10009", "Electricity", "h", "2026-06-07T08") == \
        "HN2#2|HN3#9|HN4#456|L#10009#Electricity#h#2026-06-07T08"


def test_delimiter_invariant_node_sorts_before_descendants():
    own = m.build_sk("HN2#2|HN3#9|HN4#456", "Electricity", "d", "2026-06-07")
    child = m.build_sk("HN2#2|HN3#9|HN4#456|L#10009", "Electricity", "d", "2026-06-07")
    assert own < child
